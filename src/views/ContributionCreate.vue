...
const createContribution = async () => {
  ...
  try {
    ...
    success.value = 'Contribution created successfully!'
    ...
    error.value = err.response?.data?.message || 'Failed to create contribution'
    console.error(err)
  } finally {
...
onMounted(() => {
  ...
  // Set today's date as default for datePaid
  const today = new Date().toISOString().split('T')[0]
  form.value.datePaid = today
})
</script>
